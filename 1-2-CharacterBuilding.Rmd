...
As a final step in the generation of Abilities, the player has to check the Favoured Abilities for the character's Origin. Each Origin typically has two or more Favoured Abilities, and characters whose Favoured Abilities match their Primary or Secondary Abilities gain Skill-related bonus:
...
<div class="tip">**Tip**: It is a good idea to take into account how racial Favoured Abilities match with the Primary and Secondary Abilities of each class build, when picking your Origin and Class. However, if you really like a particular Origin and Class combination with mismatched Abilities, you should not hesitate to take it - the Skill bonuses are a nice thing to have, but they won't make or break a character.</div>


<div class="compatibility">**Compatibility Note**: The Favoured Ability rule replaces racial Ability bonuses from older rulebooks. When an Origin provides a bonus to one or more Abilities, consider instead that these Abilities are Favoured Abilities.</div>
...
- **Parry**: A character wielding one or more weapons or shields with Parry gains +1 to AC against Melee and Close attacks. The bonus from multiple instances of this keyword does not stack.
...
**Armor Penalty**: Some types of armor have an Armor Penalty value (typically -1). This penalty attacks to Hit rolls of Ranged and Area attacks, as well as Skill Checks for Athletics, Endurance, Finesse, and Stealth.

<div class="tip">**Tip**: Characters focused on Ranged and Area attacks should consider carefully whether the added protection of Heavy Armor is worth taking the Armor Penalty to hit. As a rule of thumb, a +1 to Hit on all attacks is worth more than a +1 to defend against half of enemy attacks (since armor does not protect against attacks targeting Fortitude, Reflex or Will). </div>
...
A Feat is a character customization option that lets player specialize their characters beyond their Origin, Class, and Power selection. Each Feat provides a significant improvement to the character's combat capabilities. Characters start the game with one Feat, and gain additional ones as they level up, to a maximum of nine. On top of that, characters may gain bonus feats from other sources, such as their Origin or Class. Depending on the specific case, a character gaining a bonus Feat may be able of picking any available Feat of a certain level or lower, or of picking any feat within a given Feat Category, or just gain a predetermined Feat.
...
- **No class/Origin requirements**: We wanted the total number of feats to be manageable for players, but also to maintain backwards compatibility with previous Origins and Classes. Since older rulebooks have dozens of Origins and Classes, and including Feats for only a subset of them would be considered unfair and introduce balance issues, we chose to remove racial feats altogether and replace Class Feats with options based on Class Role and Power Source, which fill a similar niche with much less effort and complexity. That said, we might reconsider this for future supplements.</div>
