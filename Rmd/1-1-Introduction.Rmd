...
The general method to resolve a check is to roll a **d20**, add the character's **bonu**s to that check (if any) and any **modifiers** that apply, and compare the result with a target number. If the result is equal or greater than the target number, the check is a success, otherwise, the check is a failure.
...
To resolve a damage roll, roll all the **Damage Dice** required for the attack, and add them together. Then, add to that total any **Damage bonus** that applies for that attack. 
