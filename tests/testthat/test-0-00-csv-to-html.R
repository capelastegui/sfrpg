...
test_that("buildElement works", {
  ...
  str_result = buildElement(c('hello',NA, '', 'world'),'<<', '>>', skipEmpty=FALSE)
  str_expected = c('<<hello>>', '<<NA>>', '<<>>','<<world>>')
  expect_equal(str_result, str_expected)
  
  # Test 3 - missing values, skipEmpty=TRUE
  str_result = buildElement(c('hello',NA,'', 'world'),'<<', '>>', skipEmpty=TRUE)
  str_expected = c('<<hello>>', '', '', '<<world>>')
  expect_equal(str_result, str_expected)
...
test_that("buildElementApply works", {
  df_pre = tibble(x='<x>', y='<y>' ,Body='(')
  df_post = tibble (x= '<x>', y='</y>', Body=')')

  # Test 1 - simple input
  str_result = buildElementApply(
...
test_that("buildTableApply works", {
  df_in = tibble(x=c('hello','world'),y=c('1', '2')) 
  
  # Test 1 - simple input
  str_result = buildTableApply(df_in)
  str_expected = paste0(
    "<table> \r\n",
  ...
  str_result = buildTableApply(df_in, skipHeader=TRUE)
  str_expected = paste0(
    "<table> \r\n",
