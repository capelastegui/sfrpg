...
Monsters have fixed damage values, so the rules for their damage rolls are much
simpler:

```
Damage = Attack Damage - Resist Dice
```

**Resist Dice** : PCs receiving damage from a monster get to roll Resist Dice,
which are subtracted from the final damage value. Resist Dice depend on the
 level of the attacked PC:
...
If a monster deals damage multiple times to the same PC as part of the same
 action (for example with a Double Strike attack that rolls to hit twice), 
 add all damage together and treat it like a single attack for the purposes of
 rolling Resist Dice.
 
If a monster deals damage to another monster, no Resist Dice apply - use the
 full damage value instead.
...
For each encounter, the GM keeps track of how many turns have passed 
since the encounter started. At the start of the third turn, the GM
