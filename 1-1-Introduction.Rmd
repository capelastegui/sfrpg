...
D20s are always used for Checks. D4s are used for Action Point Rolls. Damage rolls can use d4s, d6s, d8s, d10s or d12s - the specific dice used by a character at any given type depends on that character's choice of weapons and powers.
...
To resolve a damage roll, roll all the **Damage Dice** required for the attack, and add them together. Then, add to that total the  **Ability Bonus** to damage for that attacks well as any other **Damage Bonuses** that apply. High level characters then apply multipliers to the damage dice and ability bonus based on their level:
```
...
- Classes with normal HP: 25 HP + 2.5 HP per level
- Classes with high HP: 30 HP + 3 HP per level.
- Classes with very high HP: 35 HP + 3.5 HP per level.
